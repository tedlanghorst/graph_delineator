...
def load_merit_basin_data(basin_id: int, catchment_dir: Path, river_dir: Path) -> tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
    """
    ...
    cat_file = catchment_dir / basin_subdir / f"cat_pfaf_{basin_id}_MERIT_Hydro_v07_Basins_v01.shp"
    riv_file = river_dir / basin_subdir / f"riv_pfaf_{basin_id}_MERIT_Hydro_v07_Basins_v01.shp"
...
def identify_basins_for_gauges(gauges_gdf: gpd.GeoDataFrame, megabasins_path: Path) -> dict[int, list[str]]:
    """
    ...
    if basin_field is None:
        ...
        print("Warning: Could not find basin ID field, attempting to use 'basin' column from gauges")
        if 'basin' in gauges_with_basins.columns:
            basin_gauge_map = gauges_with_basins.groupby('basin')['id'].apply(list).to_dict()
            return {int(k): v for k, v in basin_gauge_map.items()}
        else:
            raise ValueError("Could not identify basin field in megabasins shapefile and no 'basin' column in gauges")
    
    basin_gauge_map = gauges_with_basins.groupby(basin_field)['id'].apply(list).to_dict()
