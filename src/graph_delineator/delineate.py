...
class DelineationResult:
    ...
    def to_geodataframes(self, crs="EPSG:4326") -> Tuple[gpd.GeoDataFrame, gpd.GeoDataFrame, gpd.GeoDataFrame]:
        """Export graph to GeoDataFrames for visualization/saving"""
        subbasins = self._export_subbasins(crs)
        rivers = self._export_rivers(crs)
        gauges = self._export_gauges(crs)
        return subbasins, rivers, gauges
        ...
        for node_id, data in self.graph.nodes(data=True):
            if data.get('polygon') is not None:
                records.append({
                    ...
                    'nextdown': list(self.graph.successors(node_id))[0] if self.graph.out_degree(node_id) > 0 else None
                })
        return gpd.GeoDataFrame(records, crs=crs) if records else gpd.GeoDataFrame()
    
    def _export_rivers(self, crs) -> gpd.GeoDataFrame:
        records = []
        for node_id, data in self.graph.nodes(data=True):
            if data.get('river_geom') is not None:
                records.append({
                    'id': node_id,
                    'geometry': data['river_geom'],
                    'length_km': data.get('length_km', 0),
                    'strahler': data.get('strahler_order', 0),
                    'nextdown': list(self.graph.successors(node_id))[0] if self.graph.out_degree(node_id) > 0 else None
                })
        return gpd.GeoDataFrame(records, crs=crs) if records else gpd.GeoDataFrame()
        ...
        for node_id, data in self.graph.nodes(data=True):
            if data.get('is_gauge', False):
                records.append({
                    ...
                    'area_km2': data.get('area_km2', 0),
                    'uparea_km2': data.get('uparea_km2', 0),
                    'snapped_lat': data.get('snapped_lat', data['lat']),
                    'snapped_lng': data.get('snapped_lng', data['lng'])
                })
        return gpd.GeoDataFrame(records, crs=crs) if records else gpd.GeoDataFrame()
...
def get_megabasin_outlet_dict(
    ...
    gauges_with_basins = gpd.sjoin(gauges_gdf, megabasins, how='left', predicate='within')
    
    megabasin_dict = {}
    for megabasin, group in gauges_with_basins.groupby('BASIN')[['outlet_id','id']]:
        megabasin_dict[megabasin] = group.groupby('outlet_id')['id'].apply(list).to_dict()
...
def build_graph_with_geometries(
    ...
    for comid in catchments.index:
        ...
        G.add_node(
            ...
            area_km2=area_km2,
            length_km=river.get('lengthkm', river.geometry.length * 111),  # Rough conversion if needed
            node_type='original',
            is_gauge=False,
...
def insert_gauge_into_graph(
    ...
    successors = list(G.successors(original_node_id))
    ...
    G.add_node(
        ...
        lat=lat,
        lng=lng,
        snapped_lat=split_point.y if river_geom else lat,
        snapped_lng=split_point.x if river_geom else lng,
    ...
    print(f"  Inserted gauge {gauge_id}: area={gauge_area:.2f} km², downstream remainder={remainder_area:.2f} km²")
    return True
...
def insert_all_gauges(
    ...
    for _, gauge in gauges_with_nodes.iterrows():
        ...
        remainder_polygon = remainder_polygon.buffer(-MERIT_RES/2).buffer(MERIT_RES/2)  # Opening op. to clean up small errors
        
        # Insert into graph
        success = insert_gauge_into_graph(
            G, gauge_id, gauge['lat'], gauge['lng'],
            gauge_polygon, remainder_polygon, node_id
        )
...
def consolidate_graph(
    ...
    def merge_nodes(source, target, merge_type):
        ...
        if source_data.get('polygon') and target_data.get('polygon'):
            merged_polygon = unary_union([source_data['polygon'], target_data['polygon']])
        ...
        # Sum areas
        target_data['area_km2'] = target_data.get('area_km2', 0) + source_data.get('area_km2', 0)
        target_data['length_km'] = max(target_data.get('length_km', 0), source_data.get('length_km', 0))
    ...
    for iteration in range(max_iterations):
        ...
        for leaf in leaf_nodes:
            ...
            if successors:
                target = successors[0]
                combined_area = (G.nodes[leaf].get('area_km2', 0) + 
                               G.nodes[target].get('area_km2', 0))
...
def calculate_stream_orders(G: nx.DiGraph):
    ...
    for node in sorted_nodes:
        ...
        if not predecessors:
            ...
            # Calculate Strahler
            upstream_orders = [G.nodes[p].get('strahler_order', 1) for p in predecessors]
...
def calculate_upstream_areas(G: nx.DiGraph):
    ...
    for node in sorted_nodes:
        ...
        if not predecessors:
            ...
            G.nodes[node]['uparea_km2'] = upstream_total + G.nodes[node].get('area_km2', 0)
        ...
        for outlet_id, outlet_gauge_ids in outlet_dict.items():
            ...
            if consolidate:
                merge_history = consolidate_graph(
                    G,
                    max_area=max_area,
                    preserve_gauges=preserve_gauges
                )
                print(f"  Consolidation: {len(merge_history)} merges")
            
            # Store result
            results[outlet_id] = DelineationResult(
                graph=G,
                outlet_id=outlet_id
            )
...
def load_basin_data(basin_id: int, merit_dirs: Dict[str, Path]) -> Tuple[gpd.GeoDataFrame, gpd.GeoDataFrame]:
    ...
    cat_file = catchment_dir / basin_subdir / f"cat_pfaf_{basin_str}_MERIT_Hydro_v07_Basins_v01.shp"
    riv_file = river_dir / basin_subdir / f"riv_pfaf_{basin_str}_MERIT_Hydro_v07_Basins_v01.shp"
...
def save_results(results: Dict[str, DelineationResult], output_dir: str):
    ...
    for outlet_id, result in results.items():
        ...
        subbasins, rivers, gauges = result.to_geodataframes()
        
        # Save files
        if not subbasins.empty:
            subbasins.to_file(outlet_dir / "subbasins.gpkg", driver="GPKG")
        if not rivers.empty:
            rivers.to_file(outlet_dir / "rivers.gpkg", driver="GPKG")
        if not gauges.empty:
            gauges.to_file(outlet_dir / "gauges.gpkg", driver="GPKG")
        
        # Save summary
        with open(outlet_dir / "summary.txt", 'w') as f:
            f.write(f"Watershed: {outlet_id}\n")
            f.write(f"Nodes: {result.graph.number_of_nodes()}\n")
            f.write(f"Edges: {result.graph.number_of_edges()}\n")
            f.write(f"Gauges: {len(gauges)}\n")
            f.write(f"Total area: {subbasins['area_km2'].sum():.2f} km²\n")
